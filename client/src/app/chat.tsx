...
export default function Chat() {
  ...
  const [fileId, setFileId] = useState<string | null>(null); // State to store UUID
  ...
  // Scroll to the bottom when messages or isSendingMessage changes
  useEffect(() => {
  ...
  // Handle PDF upload with error handling
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      formData.append("id", uuid); // Include UUID in FormData
      ...
      setPdfFile(file);
      setFileId(uuid); // Store UUID in state
      setMessages([
  ...
  const handleSendMessage = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Make API call to /chat endpoint, including fileId
      const response = await axios.post<{ role: "assistant"; content: string }>(
        "http://localhost:3001/chat",
        {
          question: messageInput,
          userId,
          id: fileId, // Send UUID instead of filename
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
                ...
                <div className="space-y-6">
                  ...
                  {isSendingMessage && (
                    <div className="flex justify-start">
                      <div className="max-w-[85%] md:max-w-[75%] p-4 rounded-2xl bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 border border-gray-100 dark:border-gray-600">
